...
// Temporary use until we implement a better "rotating" camera
// This is a WIP just to make sure cameras work when connected to network
// CameraWork has a feature ("follow on start" -> see player prefab) that allows toggle on and off over network for offline testing...
// Thus, I am still working on using the 3rd person camera as the primary one.  
// This is from the shared scripts in Photon Networking

/// <summary>
/// We are going to Control the CameraWork behaviour by turning it off and on depending on whether the player it as to follow is the local player or not.
/// There is a public property followOnStart which we can set to true if we want to use this on a non-networked environment, for example in our test scene, 
/// or in completely different scenarios.
